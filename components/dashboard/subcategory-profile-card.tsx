...
export type SubcategoryProfileCardProps = {
  name: string;
...
export function SubcategoryProfileCard({
  name,
  categoryName,
  active,
  updatedAt,
  className,
  actions,
}: SubcategoryProfileCardProps) {
  const { t, locale } = useI18n();
  const title = name.trim() || "—";
  const secondary = categoryName?.trim();
      ...
      <div className={cn("space-y-4", actions && "pr-10 sm:pr-12")}>
        ...
        <div className="space-y-1 border-t border-border/60 pt-4">
          <p className="text-sm text-muted-foreground">
            {t("admin.subcategories.table.updatedAt")}
          </p>
          <p
