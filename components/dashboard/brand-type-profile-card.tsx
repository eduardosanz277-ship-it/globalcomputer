...
export type BrandTypeProfileCardProps = {
  name: string;
  brandName?: string | null;
  active: boolean;
  updatedAt: string;
  className?: string;
  /** Esquina superior derecha (p. ej. menú de acciones en admin móvil). */
  actions?: ReactNode;
};
...
export function BrandTypeProfileCard({
  name,
  brandName,
  active,
  updatedAt,
  className,
  actions,
}: BrandTypeProfileCardProps) {
  const { t, locale } = useI18n();
  const title = name.trim() || "—";
  const secondary = brandName?.trim();
      ...
      >
        ...
        <div className="space-y-1 border-t border-border/60 pt-4">
          <p className="text-sm text-muted-foreground">
            {t("admin.brandTypes.table.updatedAt")}
          </p>
          <p
